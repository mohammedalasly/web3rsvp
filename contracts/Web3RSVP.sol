...
///@title RSVP Dapp.
// a web3-native Eventbrite, except attendees need to deposit ETH to RSVP
...
contract Web3RSVP {
    ...
    // we’re storing information related to the creation of a new event on our RSVP contract.
    struct CreateEvent {
        bytes32 eventId;
        string eventDataCID;
        address eventOwner;
    ...
    // mapping to make sure we are referencing the correct event when we call functions on that event
    mapping(bytes32 => CreateEvent) public idToEvent;

    //function that gets executed and sets the value based on information the user passed in.
    function createNewEvent(
        ...
        // We initialize the two arrays we’ll use to track RSVPs and attendees.
        ...
        idToEvent[eventId] = CreateEvent(
            ...
            confirmedRSVPs,
            claimedRSVPs,
            false // set the boolean paidOut to false because at the time of eventCreation,
            //there have been no payouts to the RSVPers (there are none yet) or the event owner yet.
        );
